...
public class Maze implements Serializable {
    ...
    private void createSimpleMaze() {
        ...
        for (int i = 1; i <= myMazeSize; i++) {
            ...
            myRooms[i][1].getMyWestDoor().setMyDoorStatus(DoorStatus.LOCKED);
        }
        for (int i = 1; i <= myMazeSize; i++) {
            //myRooms[i][myMazeSize + 1] = new Room(i, myMazeSize + 1, false);
            myRooms[i][myMazeSize].getMyEastDoor().setMyDoorStatus(DoorStatus.LOCKED);
        }
        for (int j = 1; j <= myMazeSize; j++) {
            //myRooms[0][j] = new Room(0, j, false);
            myRooms[1][j].getMyNorthDoor().setMyDoorStatus(DoorStatus.LOCKED);
        }
        for (int j = 1; j <= myMazeSize; j++) {
            //myRooms[myMazeSize + 1][j] = new Room(myMazeSize + 1, j, false);
            myRooms[myMazeSize][j].getMySouthDoor().setMyDoorStatus(DoorStatus.LOCKED);
        }
